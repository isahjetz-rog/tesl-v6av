...
export default function NotificationToast({ theme = 'dark' }: NotificationToastProps) {
      ...
      <AnimatePresence mode="wait">
        {isVisible && (
          <motion.div
            ...
            className={`pointer-events-auto w-full border rounded-2xl p-4 shadow-xl backdrop-blur-md overflow-hidden relative transition-colors duration-300 ${
              theme === 'light'
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between space-x-2">
                  <span className={`text-xs font-bold font-sans ${theme === 'light' ? 'text-neutral-900' : 'text-white'}`}>
                    {current.name}{' '}
                    <span className={`font-normal lowercase text-[10px] ml-1 ${theme === 'light' ? 'text-neutral-500' : 'text-neutral-400'}`}>
                      {current.countryCode} {current.country}
                    </span>
                  </span>
                </div>
                <p className={`text-[11px] mt-0.5 ${theme === 'light' ? 'text-neutral-500' : 'text-neutral-400'}`}>
                ...
                <div className="flex items-center space-x-1.5 mt-2 text-emerald-500 font-semibold text-[11px]">
